...
data_file_name='Benign_v_Attack_normal.csv'
...
train_data_path=experiments_dir + 'Benign_v_Attack_normal.csv_train.csv'
x_train, y_train = load_data(train_data_path, '16')
test_data_path=experiments_dir + 'Benign_v_Attack_normal.csv_test.csv'
...
add_labels[:train_size]=y_train
	#kernel_train_sub_dial = np.diag(kernel_train_i)
	#re_dial_sub = np.tile(kernel_train_sub_dial,(train_size+add_num_per*500, 1))
	#kernel_train_nr = kernel_train_i / np.sqrt(re_dial_sub * re_dial_sub.T + 1e-10)
	#kernel_train_nr =kernel_train_nr.astype(np.float32)
...
#re_dial_train = np.tile(kernel_train_sub_dial,(test_size, 1))
#kernel_test_nr = kernel_test_ov/ np.sqrt(re_dial_test.T * re_dial_train + 1e-10)
