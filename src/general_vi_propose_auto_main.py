...
from auto_ov_classify import autocnn_ov_classify
...
value=np.zeros((16,2,6))
evalue=np.zeros((16,2,6))
...
for data_file_pre_name in l:
	...
	value[x,0,:]=d[0].T
	evalue[x,0,:]=d[1].T
	tr_size[0,x],mi_size[0,x],d=autocnn_ov_classify(data_file_dir,data_file_pre_name,experiments_dir)
	value[x,1,:]=d[0].T
	evalue[x,1,:]=d[1].T
	x=x+1
...
for data_file_pre_name in l:
	...
	print(' ')	
	print('Proposed autoencoder:')
	print('TP: %.1f FP: %.1f FN: %.1f TN: %.1f TD: %.1f FD: %.1f ' % (value[x,1,0],value[x,1,1],value[x,1,2],value[x,1,3],value[x,1,4],value[x,1,5]))
	print('Recall: %.4f Pre: %.4f TNR: %.4f F-score: %.4f G-mean: %.4f Acc: %.4f' % (evalue[x,1,0],evalue[x,1,1],evalue[x,1,2],evalue[x,1,3],evalue[x,1,4],evalue[x,1,5]))
	print('General autoencoder: ')
	print('TP: %.1f FP: %.1f FN: %.1f TN: %.1f TD: %.1f FD: %.1f ' % (value[x,0,0],value[x,0,1],value[x,0,2],value[x,0,3],value[x,0,4],value[x,0,5]))
	print('Recall: %.4f Pre: %.4f TNR: %.4f F-score: %.4f G-mean: %.4f Acc: %.4f' % (evalue[x,0,0],evalue[x,0,1],evalue[x,0,2],evalue[x,0,3],evalue[x,0,4],evalue[x,0,5]))
	print(' ')
	print(' ')	
	x=x+1	