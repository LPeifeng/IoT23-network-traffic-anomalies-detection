...
import json
import numpy as np
from helpers.log_helper import log_duration
...
def combine_imbalance_files(source_dir,
                  source_file_names,
                  output_dir,
                  output_file_name,
                  header_line=None,
                  majority_rows_from_file=None,
                  minority_rows_from_file=None,
                  skip_rows=None):
    if len(source_file_names) == 0:
        _, _, filenames = next(walk(source_dir))
        source_file_names = filenames
    ...
    for source_file_name in source_file_names:
        source_files.append(source_dir + source_file_name)
    target_file_path = output_dir + output_file_name
    combine_imbalance_files_content(source_files, target_file_path, header_line=header_line, majority_rows_from_file=majority_rows_from_file, minority_rows_from_file=minority_rows_from_file, skip_rows=skip_rows)
...
def combine_imbalance_files_content(source_files,
                          target_file_path,
                          header_line=None,
                          majority_rows_from_file=None,
                          minority_rows_from_file=None,
    ...
    with open(target_file_path, "w+") as target_file:
        if header_line is not None:
            target_file.write(header_line)
        file_counter += 1
        logging.info(str(file_counter) + '. Read data from file: ' + source_files[0])
        write_file_content(source_files[0], target_file, max_rows_from_file=majority_rows_from_file, skip_rows=skip_rows)
        if delete_source_file:
            os.remove(source_files[0])
            logging.info("Deleting file: " + fsource_files[0])
        file_counter += 1
        logging.info(str(file_counter) + '. Read data from file: ' + source_files[1])
        write_file_content(source_files[1], target_file, max_rows_from_file=minority_rows_from_file, skip_rows=skip_rows)
        if delete_source_file:
            os.remove(source_files[1])
            logging.info("Deleting file: " + source_files[1])
...
def Normal_file_content(source_dir, source_file_name):
    """
    ...
    source_file_path = source_dir + source_file_name
    output_file_path = source_file_path + '_normal.csv'

    features = [
        feature_selections['F16'],
    ]
    df = df_get(source_file_path, delimiter=',')
    label_name='detailed-label'
    s_features= features[0]['features']
    ss_features = [x for x in s_features if x not in [label_name]]
    y_data = df[label_name]
    x_data = df[ss_features]


    data_value=x_data.values
    data_rows = data_value.shape[0]
    data_cols = data_value.shape[1]
    new_data=np.zeros(shape=(data_rows,data_cols))
            
    for i in range(0, data_cols, 1):
        data_col_min_values = min(data_value[:,i])
        data_col_max_values = max(data_value[:,i])
        if data_col_max_values==data_col_min_values:
            if data_col_max_values!=0 :
                data_value[:,i]=data_value[:,i]/data_col_max_values
        else:
            data_value[:,i]=(data_value[:,i]-data_col_min_values) /(data_col_max_values-data_col_min_values)
       
    new_data = np.column_stack((data_value,y_data))
    rest=pd.DataFrame(new_data)
    write_to_csv(rest,output_file_path,mode='w') 
    ...
    exec_time_seconds = (end_time - start_time)
    exec_time_minutes = exec_time_seconds / 60
    logging.info("-----> Normalization finished in %s seconds = %s minutes ---" % (exec_time_seconds, exec_time_minutes))
