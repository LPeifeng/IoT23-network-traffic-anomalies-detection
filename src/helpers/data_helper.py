...
from iot23 import format_line, get_train_data_path, get_test_data_path
from helpers.dataframe_helper import df_get, df_transform_to_numeric, df_encode_objects, save_to_csv, write_to_csv
from helpers.file_helper import mk_dir, combine_imbalance_files, combine_files, shuffle_file_content, Normal_file_content
...
def run_imbalance_data_preprocessing(sources_dir,
                           output_dir,
                           header_line,
                           cleanup_conf,
                           data_samples=None,
    ...
    for sata_sample in data_samples:
        ...
        combined_data_file_name = sata_sample["combined_data_file_name"]
        clean_data_file_name = sata_sample["clean_data_file_name"]
        Normal_data_file_name = sata_sample["Normal_data_file_name"]
        majority_rows = sata_sample["max_rows_majority_file"]
        minority_rows = sata_sample["max_rows_minority_file"]

        exists = path.exists(output_dir + clean_data_file_name)
        if overwrite is True or not exists:
            # Combine slices from files
            combine_imbalance_files(sources_dir,
                          source_files,
                          output_dir,
                          combined_data_file_name,
                          header_line=format_line(header_line),
                          majority_rows_from_file=majority_rows,
                          minority_rows_from_file=minority_rows,
                          skip_rows=1)
            ...
            #Normalize data
            Normal_file_content(output_dir, clean_data_file_name)
