...
feature_selections = {
    ...
    # EXP_FL16_FT16_R_ / EXP_FL4_FT16_R_
    # All without:
    # 'ts', 'uid', 'label', 'id.orig_h', 'id.resp_h', 'missed_bytes'
    "F16": {
        "description": 'F16',
        "features": [
            'id.orig_p', 'id.resp_p', 'proto',
            'service', 'duration', 'orig_bytes', 'resp_bytes', 'conn_state',
            'local_orig', 'local_resp', 'history', 'orig_pkts',
            'orig_ip_bytes', 'resp_pkts', 'resp_ip_bytes', 'tunnel_parents',
            'detailed-label'
        ]},
...
def get_data_Lable_name(dataset_name='S02'):
    return Labels[dataset_name]

def get_imbalance_data_sample(dataset_name='S02', rows_majority_dataset_file=100_000, rows_minority_dataset_file=1_000):
    sample_name = dataset_name + ('_R_' + 'majority_data:'+str(format(rows_majority_dataset_file, '_d')) +'minority_data:'+str(format(rows_minority_dataset_file, '_d')))
    return {
        "description": sample_name,
        "files": datasets[dataset_name],  # empty => combine all source files
        "max_rows_majority_file": rows_majority_dataset_file,
        "max_rows_minority_file": rows_minority_dataset_file,
        "combined_data_file_name": sample_name + '.csv',
        "clean_data_file_name": sample_name + '_clean.csv',
        "Normal_data_file_name": sample_name + '_normal.csv'}
