...
l=['20-1','21-1','39-1','42-1','44-1','48-1','52-1','60-1']
x=0
alg=['Proposed','QLSVM','SMOTE','MWMOTE','EFS','S-SMOTE','B-SMOTE']

#tr_size[0,x],mi_size[0,x],d=Mwmote_svm(data_file_dir,data_file_pre_name,experiments_dir)
#Vsmote_svm(data_file_dir,data_file_pre_name,experiments_dir)

for data_file_pre_name in l:
	print('Running', data_file_pre_name, 'dataset...')
	tr_size[0,x],mi_size[0,x],d=Vsmote_svm(data_file_dir,data_file_pre_name,experiments_dir,alg[3])
	value[x,:]=d[0].T
	evalue[x,:]=d[1].T
	x=x+1
print('Results of smote_svm classification (using original features):')
x=0
for data_file_pre_name in l:
